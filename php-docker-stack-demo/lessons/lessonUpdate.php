...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $lesson_id = isset($_POST['id']) ? mysqli_real_escape_string($connect, $_POST['id']) : null;
    $name = isset($_POST['name']) ? mysqli_real_escape_string($connect, $_POST['name']) : null;

    if (!empty($name)) {
        $update_query = "UPDATE lessons SET name = '$name' WHERE id = $lesson_id";
        $update_result = mysqli_query($connect, $update_query);
        if ($update_result) {
          echo '<script>';
          echo 'alert("Course information has been updated!");';
          echo 'window.location.href = "lesson.php";';
          echo '</script>';
        } else {
...
<div class="login-page">
  <div class="form">
    ...
    <?php
    $lesson_id = isset($_GET['lessonId']) ? $_GET['lessonId'] : null;
    if (!empty($lesson_id)) {
        $lesson_id = mysqli_real_escape_string($connect, $_GET['lessonId']);
        $new_query = "SELECT * FROM lessons WHERE id = $lesson_id";
        ...
        if ($new_result) {
            while ($row = mysqli_fetch_assoc($new_result)) {
                ...
                echo '<input type="text" required name="name" value="' . $row['name'] . '"/>';
            }
        }
    }
    ?>
